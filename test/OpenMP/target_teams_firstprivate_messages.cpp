...
public:
  S4(int v) : a(v) {
#pragma omp target teams firstprivate(a) firstprivate(this->a)
    for (int k = 0; k < v; ++k)
      ++this->a;
  }
};
...
public:
  S5(int v) : a(v) {}
  S5 &operator=(S5 &s) {
#pragma omp target teams firstprivate(a) firstprivate(this->a) firstprivate(s.a) // expected-error {{expected variable name or data member of current class}}
    for (int k = 0; k < s.a; ++k)
      ++s.a;
    return *this;
  }
...
public:
  S7(T v) : a(v) {
#pragma omp target teams firstprivate(a) firstprivate(this->a) firstprivate(T::a)
    for (int k = 0; k < a.a; ++k)
      ++this->a.a;
  }
  S7 &operator=(S7 &s) {
#pragma omp target teams firstprivate(a) firstprivate(this->a) firstprivate(s.a) firstprivate(s.T::a) // expected-error 2 {{expected variable name or data member of current class}}
...
{}
#pragma omp target teams firstprivate(a, b) // expected-error {{firstprivate variable with incomplete type 'S1'}}
{}
#pragma omp target teams firstprivate(argv[1]) // expected-error {{expected variable name}}
{}
#pragma omp target teams firstprivate(e, g)
{}
...
void bar(S4 a[2]) {
#pragma omp parallel
#pragma omp target teams firstprivate(a)
  {}
}
...
#pragma omp target teams firstprivate(h) // expected-error {{threadprivate or thread local variable cannot be firstprivate}}
{}
#pragma omp target teams firstprivate(B::x) // expected-error {{threadprivate or thread local variable cannot be firstprivate}}
{}
#pragma omp target teams shared(i)
#pragma omp parallel
  {
...
#pragma omp target teams firstprivate(i)
  {}
  static int si;
#pragma omp target teams firstprivate(si) // OK
  {}
#pragma omp target teams map(i) firstprivate(i) // expected-error {{firstprivate variable cannot be in a map clause in '#pragma omp target teams' directive}}
  {}
