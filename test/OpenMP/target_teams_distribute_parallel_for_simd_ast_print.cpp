...
// RxUN: %clang_cc1 -fopenmp -x c++ -std=c++11 -emit-pch -o %t %s
// RxUN: %clang_cc1 -fopenmp -std=c++11 -include-pch %t -fsyntax-only -verify %s -ast-print | FileCheck %s
...
// CHECK-NEXT: *a = 2;
#pragma omp target teams distribute parallel for simd private(argc,b),lastprivate(d,f),reduction(+:e) reduction(min : g),  collapse(2) dist_schedule(static, 3) default(none) firstprivate(f) num_teams(b) num_threads(argc) shared(k1, k2) map(to:b) map(from:c) map(tofrom:d) device(f) depend(in: argc) depend(out: c) depend(inout: d) if(g)
  for (int i = 0; i < 10; ++i)
