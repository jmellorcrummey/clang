...
//#pragma omp target
//#pragma omp teams
//#pragma omp distribute parallel for dist_schedule (static, argv[1]=2) // Nexpected-error {{expected ')'}} Nexpected-note {{to match this '('}} Nexpected-error3 {{expression must have integral or unscoped enumeration type, not 'char *'}}
//  for (int i = 0; i < 10; ++i) foo();
  return T();
}
