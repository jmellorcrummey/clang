...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  ...
  Compilation *C = new Compilation(*this, *HostToolChain, UArgs.release(),
                                   TranslatedArgs);
    ...
    for (SmallVectorImpl<phases::ID>::iterator
      ...
      if ( Phase > phases::Preprocess && PreviousTotalTargets < TotalTargets &&
        ...
        for (unsigned tgt=0; tgt<OpenMPToolChainNum; ++tgt){
          ...
          TgtIA.get()->setOffloadingDevice(OpenMPTargetToolChains[tgt]);

          BindTargetAction *BindedTgtIA =
               new BindTargetAction(std::move(TgtIA), OpenMPTargetToolChains[tgt]);
        ...
        if (tgt == 0 && InjectCuda && Phase == CudaInjectionPhase &&
          ...
          ActionsForTarget[tgt].reset(
            buildCudaActions(*this, TC, Args, InputArg,
                             std::move(ActionsForTarget[tgt]), Actions)
                .release());

          if (!ActionsForTarget[tgt].get()) {
            NothingElseToDo = true;
            break;
          }
        }
...
static const Tool *selectToolForJob(Compilation &C, bool SaveTemps,
  ...
  if (TC->useIntegratedAs() && !SaveTemps &&
      ...
      PrevSingleAction && isa<BackendJobAction>(PrevSingleAction)) {
  ...
  // (irrelevant since we don't support combine yet). If we need to use a
  // different toolchain for his action we cannot use the integrated assembler.
  if (PrevSingleAction && isa<PreprocessJobAction>(PrevSingleAction) &&
      !C.getArgs().hasArg(options::OPT_no_integrated_cpp) &&
      !C.getArgs().hasArg(options::OPT_traditional_cpp) && !SaveTemps &&
      !C.getArgs().hasArg(options::OPT_rewrite_objc) &&
      ToolForJob->hasIntegratedCPP() &&
      !(PrevSingleAction->getOffloadingDevice() &&
        TC->UseHostToolChainInstead(cast<JobAction>(PrevSingleAction))))
    Inputs = &PrevSingleAction->getInputs();
...
void Driver::BuildJobsForAction(Compilation &C, const Action *A,
  ...
  bool HasTargetSuffixApended =
      InputInfos[0].hasTargetSuffixApended();
  ...
  if (JA->getType() == types::TY_Nothing)
    ...
    Result = InputInfo(GetNamedOutputPath(C, *JA, BaseInput, BoundArch,
                                          ...
                                          HasTargetSuffixApended),
                       A, BaseInput, HasTargetSuffixApended);

  OutputsForActionCache[A][TC] = Result;
...
const char *Driver::GetNamedOutputPath(Compilation &C, const JobAction &JA,
  ...
  if (JA.getType() == types::TY_Object &&
    ...
    if (!types::appendSuffixForType(JA.getType())){
      End = BaseName.rfind('.');
      // If this is an OpenMP target job that already contains the target
      // suffix we need to skip both type and target suffix
      if ( JA.getOffloadingDevice() && !HasTargetSuffixApended )
...
bool Driver::ShouldUseClangCompiler(const JobAction &JA) const {
  ...
  // only handle single inputs for now except for OpenMP target compile phases).
