...
static OMPCapturedExprDecl *buildCaptureDecl(Sema &S, IdentifierInfo *Id,
  ...
  auto *CED =
      OMPCapturedExprDecl::Create(C, S.CurContext, Id, Ty, CaptureLevel);
  if (!WithInit)
...
static bool CheckNestingOfRegions(Sema &SemaRef, DSAStackTy *Stack,
  ...
  // | target teams     | master          | +                                  |
  // | target teams     | critical        | +                                  |
  // | target teams     | simd            | +                                  |
  // | target teams     | sections        | +                                  |
  // | target teams     | section         | +                                  |
  // | target teams     | single          | +                                  |
  // | target teams     | parallel for    | *                                  |
  // | target teams     |parallel for simd| *                                  |
  // | target teams     |parallel sections| *                                  |
  // | target teams     | task            | +                                  |
  // | target teams     | taskyield       | +                                  |
  // | target teams     | barrier         | +                                  |
  // | target teams     | taskwait        | +                                  |
  // | target teams     | taskgroup       | +                                  |
  // | target teams     | flush           | +                                  |
  // | target teams     | ordered         | +                                  |
  // | target teams     | atomic          | +                                  |
  // | target teams     | target          |                                    |
  // | target teams     | target parallel |                                    |
  // | target teams     | target parallel |                                    |
  // |                  | for             |                                    |
  // | target teams     | target enter    |                                    |
  // |                  | data            |                                    |
  // | target teams     | target exit     |                                    |
  // |                  | data            |                                    |
  // | target teams     | teams           |                                    |
  // | target teams     | cancellation    |                                    |
  // |                  | point           |                                    |
  // | target teams     | cancel          |                                    |
  // | target teams     | taskloop        | +                                  |
  // | target teams     | taskloop simd   | +                                  |
  // | target teams     | distribute      | !                                  |
  // | target teams     | distribute      | !                                  |
  // |                  | parallel for    |                                    |
...
StmtResult Sema::ActOnOpenMPTargetDirective(ArrayRef<OMPClause *> Clauses,
  ...
  if(getLangOpts().OpenMPImplicitDeclareTarget){
	  ...
	  // Revealed function calls are marked with OMPDeclareTargetDeclAttr attribute,
	  // in case -fopenmp-implicit-declare-target extension is enabled.
	  CheckFunctionCalls FunctionCallChecker(*this);
...
static void checkDeclInTargetContext(SourceLocation SL, SourceRange SR,
  ...
  if (isa<TagDecl>(D)) {
    ...
    // to do anything else.
    if (LD == D) {
      // Here, unless these declarations are not specified inside of declare target
      // they are not valid OpenMP 4.5, we thus give warning.
      // In case -fopenmp-implicit-declare-target these declarations are automatically
...
bool Sema::CheckFunctionCalls::TraverseLambdaCapture(LambdaExpr *LE, const LambdaCapture *C) {
...
bool Sema::CheckFunctionCalls::VisitCallExpr(CallExpr *Call) {
