...
bool CGOpenMPRuntime::ignoreSharedRegion(bool isParallel) { return false; }
...
void CGOpenMPRuntime::addToSharedRegion(llvm::Value *V, unsigned NestingLevel,
                                        bool isParallel) {
  if (ignoreSharedRegion(isParallel))
    return;
...
llvm::Constant *CGOpenMPRuntime::Get_reduce_nowait() {
  ...
  llvm::FunctionType *FT = llvm::FunctionType::get(
	...
	//new interface to handle array reduction in OpenMP4.1
  return CGM.CreateRuntimeFunction(FT, "__kmpc_reduce_nowait41");
}
...
llvm::Constant *CGOpenMPRuntime::Get_reduce() {
  ...
  llvm::FunctionType *FT = llvm::FunctionType::get(
      llvm::TypeBuilder<int32_t, false>::get(C), Params, false);
	//new interface to handle array reduction in OpenMP4.1
  return CGM.CreateRuntimeFunction(FT, "__kmpc_reduce41");
}
// Special processing for __kmpc_reduce_combined
// DEFAULT_GET_OPENMP_FUNC(reduce_combined)
llvm::Constant *CGOpenMPRuntime::Get_reduce_combined() {
  llvm::LLVMContext &C = CGM.getLLVMContext();
  llvm::Type *Params[] = {llvm::TypeBuilder<ident_t *, false>::get(C)};

  llvm::FunctionType *FT = llvm::FunctionType::get(
      llvm::TypeBuilder<int32_t, false>::get(C), Params, false);
  return CGM.CreateRuntimeFunction(FT, "__kmpc_reduce_combined");
}
...
llvm::Value *CGOpenMPRuntime::GetAtomicFuncArrayGeneral(CodeGenFunction &CGF,
    QualType QTyRes, QualType QTyIn, CGOpenMPRuntime::EAtomicOperation Aop,
    bool Capture, bool Reverse) {
  SmallString<40> Str;
  llvm::raw_svector_ostream OS(Str);
  ...
  int64_t TySize =
      CGF.CGM.GetTargetTypeStoreSize(CGF.ConvertTypeForMem(QTyRes)).getQuantity();
  if (QTyRes->isRealFloatingType()) {
    OS << "__array_atomic_float";
    if (TySize != 4 && TySize != 8 && TySize != 10 && TySize != 16)
      return 0;
  } else if (QTyRes->isComplexType()) {
    OS << "__array_atomic_cmplx";
    if ( TySize != 8 && TySize != 16)
      return 0;
  } else if (QTyRes->isScalarType()) {
    OS << "__array_atomic_fixed";
    if (TySize != 1 && TySize != 2 && TySize != 4 && TySize != 8)
      return 0;
  } else
    return 0;
...
llvm::Value *CGOpenMPRuntime::GetAtomicFunc(CodeGenFunction &CGF, QualType QTy,
  ...
  if (QTy->isArrayType()) {
     ...
     return GetAtomicFuncArrayGeneral(CGF, ElementQTy, ElementQTy, Aop, false, false);
  } else {
...
bool CGOpenMPRuntime::requiresSpecialThreadPrivateHandling() { return true; }
...
void CGOpenMPRuntime::EmitSimdInitialization(llvm::Value *&LoopIndex,
  ...
  Builder.CreateStore(LoopStart, LoopIndex);
}
...
CGOpenMPRuntime::GetOffloadEntryMangledNameForTargetRegion(unsigned &Order,
  ...
  assert(!CurTargetParentFunctionInfo.empty() &&
         "No parent function name exist!");
  auto I = TargetRegionsOrder.find(CurTargetParentFunctionInfo.front().first);
...
void CGOpenMPRuntime::registerTargetRegion(const Decl *D, llvm::Function *Fn) {
  ...
  assert(!CurTargetParentFunctionInfo.empty() &&
         "A Parent function must be registered when not in target mode!");
  TargetRegionsOrder[CurTargetParentFunctionInfo.front().first].push_back(
...
void CGOpenMPRuntime::registerLambda(const CXXRecordDecl *LR) {
  assert(LR->isLambda() && "Expecting lambda record.");

  // Register the lambda for target codegen
  OtherFunctions.insert(
      CGM.getMangledName(LR->getLambdaCallOperator())
          );

  // Recursively look for nested lambdas
  for (Decl *DC : LR->getLambdaCallOperator()->decls()) {
...
//    if (RD->isLambda()) {
//      // Check to see if this lambda is defined in the context of a target
//      // region
//      const DeclContext *DC = RD->getDeclContext();
//      while (DC) {
//        if (DC->getDeclKind() == Decl::Captured) {
...
/// \riefHelper to dump the information about a given target region to the
/// the screen if so was requested by the user.
void CGOpenMPRuntime::DumpTargetRegionInfo(const OMPExecutableDirective &D,
  ...
  const FunctionDecl *FD = CurTargetParentFunctionInfo.front().second;
  ...
  llvm::raw_svector_ostream OS(KernelListDump);
  OS << "  " << TargetRegionName << "  :  " << PLoc.getFilename() << ", '"
     << Name << "', line " << PLoc.getLine() << ", column " << PLoc.getColumn()
     << "\n";
  return;
}
...
llvm::Value * CGOpenMPRuntime::Get_omp_get_thread_num() {
    return CGM.CreateRuntimeFunction(
       llvm::TypeBuilder<omp_get_thread_num, false>::get(
           CGM.getLLVMContext()), "omp_get_thread_num");
  }
...
class CGOpenMPRuntime_NVPTX: public CGOpenMPRuntime {
  ...
  enum OMPRegionTypes {
    ...
    OMP_Sequential,
    OMP_Simd,
    OMP_SequentialSimd, // this is used when we cannot use simd behavior
  ...
  // Variable that keeps the number of parallel regions nesting. It is
  ...
  // number of OMP threads when SIMD is present
  llvm::GlobalVariable *OmpNumThreads;
  ...
  // OMP Thread ID for this CUDA thread
  llvm::AllocaInst *OmpThreadNum;

  // CUDA Global Thread ID
  llvm::AllocaInst *CudaGlobalThreadId;
  ...
  /// Generate instructions for a complex combination of constructs
  /// including loop nests.
  void
  EmitOMPCombinedNestDirectiveLoop(OpenMPDirectiveKind DKind,
                                   OpenMPDirectiveKind SKind,
                                   const OMPExecutableDirective &S,
                                   CodeGenFunction &CGF,
                                   StringRef TgtFunName,
    ...
    llvm::BasicBlock *BodyCombinedFor = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".body.combined.nest.simd.for", CGF.CurFn);
    llvm::BasicBlock *StartRegionS1 = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".start.region.s.one", CGF.CurFn);
    llvm::BasicBlock *IncCombinedFor = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".inc.combined.nest.simd.for", CGF.CurFn);
    ...
    CGF.SyncAfterCombinedBlock = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".sync.after.combined", CGF.CurFn);
    EndTarget = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".end.combined.nest.simd.for.and.target", CGF.CurFn);
    ...
    //
    // The thread ID computing the UB and LB must be the OMP thread which in this case
    // is the warp master.
    //
    // The loop LB and UB must be computed by thread with global ID 0 (for now, let all threads do it)
    // Loop UB:
...
*/
  ...
  void
  EnterOMPCombinedNestDirectiveLoop(OpenMPDirectiveKind DKind,
                                    OpenMPDirectiveKind SKind,
                                    const OMPExecutableDirective &S,
                                    CodeGenFunction &CGF,
                                    StringRef TgtFunName,
    ...
    // Function which finished off the region is called in ExitTargetLoop
  ...
  GenerateSimplifiedLoop(const OMPExecutableDirective &S,
    ...
    std::string levelRegion = std::to_string(CGF.k) + "." + std::to_string(CGF.regionID);
    llvm::BasicBlock *StartCombinedFor = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".start."+levelRegion+".for", CGF.CurFn);
    ...
    llvm::BasicBlock *BodyCombinedFor = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".body."+levelRegion+".for", CGF.CurFn);
    ...
    llvm::BasicBlock *IncCombinedFor = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".inc."+levelRegion+".for", CGF.CurFn);

    // No sync required at the end for Leaf Nodes
    //if (isNonTerminalOpenMPNode){
    //  CGF.SyncAfterCombinedBlock = llvm::BasicBlock::Create(
    //      CGF.CGM.getLLVMContext(), ".sync.after."+levelRegion, CGF.CurFn);
    //}

    // For loop end.
    llvm::BasicBlock *EndLoop = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".end."+levelRegion+".for", CGF.CurFn);
    ...
    if (const OMPTargetTeamsDistributeParallelForDirective *D =
      ...
      Arr = D->getCounters();
      numCollapsed = D->getCollapsedNumber();
    } else if (const OMPParallelForDirective *D =
         dyn_cast<OMPParallelForDirective>(&S)){
      IterVar = D->getNewIterVar();
      UBExpr = D->getNewIterEnd();
      Arr = D->getCounters();
      numCollapsed = D->getCollapsedNumber();
    } else if (const OMPParallelForSimdDirective *D =
         dyn_cast<OMPParallelForSimdDirective>(&S)){
      IterVar = D->getNewIterVar();
      UBExpr = D->getNewIterEnd();
      Arr = D->getCounters();
      numCollapsed = D->getCollapsedNumber();
    } else if (const OMPSimdDirective *D =
         dyn_cast<OMPSimdDirective>(&S)){
      IterVar = D->getNewIterVar();
      UBExpr = D->getNewIterEnd();
      Arr = D->getCounters();
    ...
    // Init Private var for loop IDX
    llvm::AllocaInst *Private = CGF.CreateMemTemp(QTy, ".idx."+levelRegion);
    ...
    for (unsigned I = 0; I < numCollapsed; ++I) {
      ...
      QualType QTy = Arr[I]->getType();
      llvm::AllocaInst *Private =
          CGF.CreateMemTemp(QTy, CGF.CGM.getMangledName(VD) + ".private."+levelRegion);
      CGF.CGM.OpenMPSupport.addOpenMPPrivateVar(VD, Private);
      llvm::BasicBlock *PrecondBB = llvm::BasicBlock::Create(
              CGF.CGM.getLLVMContext(), "omp.loop.precond."+levelRegion);
      if (isa<DeclStmt>(For->getInit()))
        CGF.EmitAnyExprToMem(VD->getAnyInitializer(), Private,
                         VD->getType().getQualifiers(), true);
      else
        CGF.EmitStmt(For->getInit());
      CGF.EmitBranchOnBoolExpr(For->getCond(), PrecondBB, EndLoop, 0);
      CGF.EmitBlock(PrecondBB);
    }
    ...
    // Open a new region whatever the node we are on.
    // An OpenMP region has a start block and an end block.
    CGF.regionID++;
    StartOpenMPRegion = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".start.omp.region."+std::to_string(CGF.regionID), CGF.CurFn);
    llvm::BasicBlock *endOpenMPRegion = llvm::BasicBlock::Create(
        CGF.CGM.getLLVMContext(), ".end.omp.region."+std::to_string(CGF.regionID), CGF.CurFn);

    // Finished the previous OpenMP loop
    ...
    // Jump to End of OpenMP region if are not the master of the parent
    // parallel unit.
    llvm::Value *isNotExecutingOpenMPRegion =
        Builder.CreateICmpNE(Builder.CreateLoad(CGF.Tid[CGF.kparent]),
                             Builder.getInt32(0));
    Builder.CreateCondBr(isNotExecutingOpenMPRegion, CGF.EndOpenMPRegion, StartOpenMPRegion);
    Builder.SetInsertPoint(StartOpenMPRegion);
    printf("   =====> Start OpenMP Region %d\n", CGF.regionID);
  ...
  EnterParalleLoopNest(OpenMPDirectiveKind DKind,
    ...
    for(unsigned int i = 1; i < CGF.p - 1; i++){
  ...
  void EnterSimdRegion(CodeGenFunction &CGF, ArrayRef<OMPClause *> Clauses) {
    ...
    // conditions under which we use only 1 lane:
    // 1. there is not a reduction clause on the simd
    // 2. the simd is not nested within a pragma #for
    // 3. there is no conditional control-flow to reach simd from parallel
    // or target
    ...
    if (!CGF.combinedSimd){
      ...
      // In case we are in the combinedSimd case we have to provide
      // a region for all the threads not taking the SIMD to go to.
      const std::string SyncAfterSimdName = ".sync.after.simd.";
      CGF.SyncAfterSimdBlock = llvm::BasicBlock::Create(
          CGM.getLLVMContext(), SyncAfterSimdName, CGF.CurFn);
    }
    ...
    llvm::Value *IsLaneInExcess = Bld.CreateICmpSGE(
    ...
    if (!CGF.combinedSimd){
      ...
      Bld.CreateCondBr(IsLaneInExcess, CGF.SyncAfterSimdBlock,
                       LaneNotInExcessBlock);
    }
...
//     map.insert(std::make_pair("nexttowardf", ""));
   ...
   void EmitSimdInitialization(llvm::Value *&LoopIndex,
                               llvm::Value *&LoopStart,
                               llvm::Value *&LoopCount,
                               OpenMPDirectiveKind DKind,
                               CodeGenFunction &CGF) {
     ...
     // Don't do this for *distribute_simd, because in that case
     // SIMD lanes are all CUDA threads (so they cross warps) and the variables
     // must be placed in shared memory.
     bool SimdWithinWarp = DKind == OMPD_parallel_for_simd ||
         DKind == OMPD_for_simd ||
     ...
     if (SimdWithinWarp) {
         ...
         LoopStart = Builder.CreateCall(Get_shuffle(LoopStart->getType()),
             Args1);
         llvm::Value *Args2[] = {LoopCount, Builder.getInt32(0),
             Builder.getInt32(WARP_SIZE)};
         LoopCount = Builder.CreateCall(Get_shuffle(LoopCount->getType()),
             Args2);
     }
  ...
  void PostProcessPrintfs(llvm::Module &M) {
    ...
    for (llvm::Value::use_iterator UI = PrintfFunc->use_begin(),
      ...
      if (BufSize == 0) {
        ...
        BufferPtr = new llvm::AllocaInst(
            llvm::Type::getInt8Ty(M.getContext()),
            llvm::ConstantInt::get(Int32Ty, BufSize), "buf_for_vprintf_args",
            Call->getParent()->getParent()->getEntryBlock().begin());
      } else {
        ...
        BufferPtr = new llvm::AllocaInst(
            PointeeType, llvm::ConstantInt::get(Int32Ty, BufSize), AlignOfFirst,
            "buf_for_vprintf_args",
            Call->getParent()->getParent()->getEntryBlock().begin());
...
private:
  ...
  llvm::Value * GetTeamReduFuncArrayGeneral(CodeGenFunction &CGF,
    QualType QTyRes, QualType QTyIn, CGOpenMPRuntime::EAtomicOperation Aop) {
  SmallString<40> Str;
  llvm::raw_svector_ostream OS(Str);
  ...
  int64_t TySize =
      CGF.CGM.GetTargetTypeStoreSize(CGF.ConvertTypeForMem(QTyRes)).getQuantity();
  if (QTyRes->isRealFloatingType()) {
    OS << "__gpu_array_warpBlockRedu_float";
    if (TySize != 4 && TySize != 8 && TySize != 10 && TySize != 16)
      return 0;
  } else if (QTyRes->isComplexType()) {
    OS << "__gpu_array_warpBlockRedu_cmplx";
    if ( TySize != 8 && TySize != 16)
      return 0;
  } else if (QTyRes->isScalarType()) {
    OS << "__gpu_array_warpBlockRedu_fixed";
    if (TySize != 1 && TySize != 2 && TySize != 4 && TySize != 8)
      return 0;
  } else
    return 0;
  ...
  SmallVector<llvm::Type *, 2> Params;
  //FIXME: type convertion for Atomic operation can be done when the 
  // 	loop is generated in codegen, not in runtime library
  llvm::Type *TyRes = CGF.ConvertTypeForMem(GetAtomicType(CGF, QTyRes));
  Params.push_back(TyRes->getPointerTo());
  Params.push_back(CGF.Int64Ty);
  llvm::Type *RetTy = CGF.VoidTy;;
  llvm::FunctionType *FunTy = llvm::FunctionType::get(RetTy, Params, false);
  return CGF.CGM.CreateRuntimeFunction(FunTy, OS.str());
...
public:
  llvm::Value *GetTeamReduFunc(CodeGenFunction &CGF, QualType QTy,
    ...
    if (QTy->isArrayType()) {
     const ArrayType *QAT = QTy->getAsArrayTypeUnsafe();
     QualType ElementQTy = QAT->getElementType();
     return GetTeamReduFuncArrayGeneral(CGF, ElementQTy, ElementQTy, Aop);
    } else {
  ...
  bool IsCompileAssignedWarp(const OMPExecutableDirective &S) {
    return false;
  }
...
///===---------------
///
/// PowerPC OpenMP Runtime Implementation
///
///===---------------

/// Target specific runtime hacks
class CGOpenMPRuntime_PowerPC : public CGOpenMPRuntime {

public:

  CGOpenMPRuntime_PowerPC(CodeGenModule &CGM) : CGOpenMPRuntime(CGM) {}
  ...
  llvm::Value *CreateOpenMPThreadPrivateCached(const VarDecl *VD,
    ...
    if (!CGM.getTarget().isTLSSupported())
      return CGOpenMPRuntime::CreateOpenMPThreadPrivateCached(VD, Loc, CGF,
                                                              NoCast);

    // In PowerPC marking the variable as thread local has shown to be much
    // faster than relying on the runtime to manage that. Therefore we override
    // the default implementation.
    if (CGM.OpenMPSupport.hasThreadPrivateVar(VD)) {
      llvm::Type *VDTy = CGM.getTypes().ConvertTypeForMem(VD->getType());
      llvm::PointerType *PTy = llvm::PointerType::get(
          VDTy, CGM.getContext().getTargetAddressSpace(VD->getType()));

      // FIXME: The original implementation specializes for static declarations
      // but that should not be required here.
      llvm::Constant *C = CGM.GetAddrOfGlobal(VD);

      if (NoCast)
        return C;
      return CGF.Builder.CreateBitCast(C, PTy);
    }

    return nullptr;
  }

  bool requiresSpecialThreadPrivateHandling() {
    return !CGM.getTarget().isTLSSupported();
  }
...
CGOpenMPRuntime *CodeGen::CreateOpenMPRuntime(CodeGenModule &CGM) {
  ...
  case llvm::Triple::ppc64le:
    return new CGOpenMPRuntime_PowerPC(CGM);
  }
