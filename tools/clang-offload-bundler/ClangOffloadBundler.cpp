...
static cl::OptionCategory ClangOffloadBundlerCategory("Clang-offload-bundler options");

static cl::list<std::string> InputFileNames("inputs", cl::CommaSeparated, cl::OneOrMore, cl::desc("[<input file>,...]"),
                                       cl::cat(ClangOffloadBundlerCategory));
static cl::list<std::string> OutputFileNames("outputs", cl::CommaSeparated, cl::OneOrMore, cl::desc("[<output file>,...]"),
                                       cl::cat(ClangOffloadBundlerCategory));
static cl::list<std::string> TargetNames("omptargets", cl::CommaSeparated, cl::OneOrMore, cl::desc("[<target triple>,...]"),
                                       cl::cat(ClangOffloadBundlerCategory));
...
class FileHandler {
protected:
  /// \brief Update the file handler with information from the header of the bundled file
  virtual void ReadHeader(MemoryBuffer &Input) = 0;
  /// \brief Read the marker of the next bundled to be read in the file. The triple of the target associated with that bundled is returned. An empty string is returned if there are no more bundles to be read.
  ...
  /// \brief Write the header of the bundled file to \a OS based on the information gathered from \a Inputs.
  virtual void WriteHeader(raw_fd_ostream &OS, ArrayRef<std::unique_ptr<MemoryBuffer>> Inputs) = 0;
  /// \brief Write the marker that initiates a bundle for the triple \a TargetTriple to \a OS.
  virtual void WriteBundleStart(raw_fd_ostream &OS, StringRef TargetTriple) = 0;
  /// \brief Write the marker that closes a bundle for the triple \a TargetTriple to \a OS.
...
public:
  ...
  bool Bundle(){
    ...
    // Open input files.
    std::vector<std::unique_ptr<MemoryBuffer> > InputBuffers(InputFileNames.size());

    unsigned Idx = 0;
    for (auto I : InputFileNames){
      ErrorOr<std::unique_ptr<MemoryBuffer>> CodeOrErr = MemoryBuffer::getFileOrSTDIN(I);
    ...
    for (auto Triple = TargetNames.begin(); Triple < TargetNames.end(); ++Triple, ++Input){
  ...
  bool Unbundle(){
    // Open Input file.
    ErrorOr<std::unique_ptr<MemoryBuffer>> CodeOrErr = MemoryBuffer::getFileOrSTDIN(InputFileNames.front());
    if (std::error_code EC = CodeOrErr.getError()) {
      llvm::errs() << EC.message() << "\n";
      return true;
    }
    ...
    for (auto Triple = TargetNames.begin(); Triple < TargetNames.end(); ++Triple, ++Output)
      Worklist[*Triple] = *Output;

    // Read all the bundles that are in the work list, and return an error is a given bundle wasn't found.
    while (!Worklist.empty()) {
      StringRef CurTriple = ReadBundleStart(Input);

      if (CurTriple.empty()){
        llvm::errs() << "error: Unable to find bundles for all requested targets.\n";
        return true;
      }
...
/// \brief Write and write 8-byte integers to/from a buffer in little-endian format.
...
protected:
  void ReadHeader(MemoryBuffer &Input){
    ...
    for (uint64_t i=0; i<NumberOfBundles; ++i){
      ...
      assert(BundlesInfo.find(Triple) == BundlesInfo.end() && "Triple is duplicated??");
  ...
  void WriteHeader(raw_fd_ostream &OS, ArrayRef<std::unique_ptr<MemoryBuffer>> Inputs){
...
public:
  TextFileHandler(StringRef Comment) : FileHandler(), Comment(Comment), ReadChars(0) {
...
int main(int argc, const char **argv) {
  ...
  if (Unbundle){
    if (InputFileNames.size() != 1){
      Error = true;
      llvm::errs() << "error: only one input file supported in unbundling mode.\n";
    }
    if (OutputFileNames.size() != TargetNames.size()){
      Error = true;
      llvm::errs() << "error: number of output files and targets should match in unbundling mode.\n";
    }
  } else {
    if (OutputFileNames.size() != 1){
      Error = true;
      llvm::errs() << "error: only one output file supported in bundling mode.\n";
    }
    if (InputFileNames.size() != TargetNames.size()){
      Error = true;
      llvm::errs() << "error: number of input files and targets should match in bundling mode.\n";
    }
  }
